public class FlourPackProblem {
    ...
    private static boolean canPack(int bigCount, int smallCount, int goal) {
        ...
        // * Make sure the params are not less than 0.
        if (bigCount >= 0 && smallCount >= 0 && goal >= 0) {
        
            // * multiply bigCount * 5 to accurately represent kilos.
            int newBigCount = bigCount * 5;

            if ((newBigCount + smallCount) < goal) {

                return false;

            };

            // * Remainder of the goal has to be less than small count.
